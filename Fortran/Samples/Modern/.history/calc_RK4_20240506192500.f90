...
subroutine calc_RK4
    ...
    do i = 1, nkoss
        ...
        k(2,1,1) = calc_acceleration(posx(i) + k(1,2,1)*0.5, velx(i) + k(1,1,1)*0.5) * dt ! k_2u
        k(2,1,2) = calc_acceleration(posy(i) + k(1,2,1)*0.5, vely(i) + k(1,1,1)*0.5) * dt ! k_2v
        k(2,1,3) = calc_acceleration(posz(i) + k(1,2,1)*0.5, velz(i) + k(1,1,1)*0.5) * dt ! k_2w
        k(2,2,1) = velx(i) * dt ! k_2x
        k(2,2,2) = vely(i) * dt ! k_2y
        k(2,2,3) = velz(i) * dt ! k_2z

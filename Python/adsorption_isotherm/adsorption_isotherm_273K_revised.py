...
def calc_uptake(phi):
    ...
    for p in press:
        if p == 0:
            ...
            path = f"E:\\LAMMPS\\GCMC_revised\\T={T}K\\phi={phi}\\EPM2F_T={T}K_P={p}bar_revised\\data\\Ngas.dat"
...
for phi in fugacity_coeff:
    ...
    plt.plot(press, uptake, marker='o', color=color, linestyle='-', label=rf"$\phi = {f}$")

...
fig_path = f'E:/Graduation Thesis/Figures/angle_dist_P={press}bar.svg'
...
for i in range(3):
    # Path
    input_path = f"E:/LAMMPS/MD/T={T[i]}K_phi={phi[i]}_P={press}bar/data/angles.dat"
    modified_path = f"E:/LAMMPS/MD/T={T[i]}K_phi={phi[i]}_P={press}bar/data/angles_modified.dat"
    ...
    sb.histplot(data=datalist, bins=bins, color=color[i], alpha=0.5, element="poly", lw=0, ax=ax, label=rf"T={T[i]} K, $\phi$={phi[i]}")

    ax.set_xlabel(r"angle [$\rm{deg}$]", fontsize=15)

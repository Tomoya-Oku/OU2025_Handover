...
fig_path = f'E:/Graduation Thesis/Figures/ke_T=273K_revised.svg'

n = 5000 # used to obtain mean value 
T = 273
press = np.array([1, 2, 3, 4, 5, 6, 7])
fugacity_coeff = np.array(["0.4", "0.45", "0.5"])
...
for f in fugacity_coeff:
    for p in press:
        path = f"E:/LAMMPS/GCMC_revised/T={T}K/phi={f}/EPM2F_T={T}K_P={p}bar_revised/data/energy_gas.dat"
...
for i, ax in enumerate(axes.flat):
    ...
    if 0 <= i and i <= 6:
        fugacity = fugacity_coeff[0]
    elif 7 <= i and i <= 13:
        fugacity = fugacity_coeff[1]
    elif 14 <= i and i <= 20:
        fugacity = fugacity_coeff[2]
